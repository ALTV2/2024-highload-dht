...
final class TableSet {
    ...
    Iterator<EntryWithTime<MemorySegment>> get(
        ...
        // MemTable goes first
        final Iterator<EntryWithTime<MemorySegment>> memTableIterator =
        ...
        // Then goes flushing
        if (flushingTable != null) {
            final Iterator<EntryWithTime<MemorySegment>> flushingIterator =
        ...
        for (int i = 0; i < ssTables.size(); i++) {
            ...
            final Iterator<EntryWithTime<MemorySegment>> ssTableIterator =
                    ssTable.get(from, to);
    ...
    EntryWithTime<MemorySegment> get(final MemorySegment key) {
        // Slightly optimized version not to pollute the heap

        // First check MemTable
        EntryWithTime<MemorySegment> result = memTable.get(key);
        if (result != null) {
            // Transform tombstone
            return result;
        }
...
//    private static EntryWithTime<MemorySegment> swallowTombstone(final EntryWithTime<MemorySegment> entry) {
//        return entry.value() == null ? null : entry;
//    }

    EntryWithTime<MemorySegment> upsert(final EntryWithTime<MemorySegment> entry) {
        return memTable.upsert(entry);
    }

    Iterator<EntryWithTime<MemorySegment>> allSSTableEntries() {
        final List<WeightedPeekingEntryIterator> iterators =
