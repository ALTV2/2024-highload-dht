...
final class SSTable {
    ...
    EntryWithTime<MemorySegment> get(final MemorySegment key) {
        ...
        if (valueLength == SSTables.TOMBSTONE_VALUE_LENGTH) {
            ...
            return new EntryWithTime<>(key, null, timeStamp);
        } else {
            // Get value
            offset += Long.BYTES;
            final MemorySegment value = data.asSlice(offset, valueLength);
            return new EntryWithTime<>(key, value, timeStamp);
        }
    }

    private final class SliceIterator implements Iterator<EntryWithTime<MemorySegment>> {
        private long offset;
